...
vim.api.nvim_create_autocmd({"BufEnter", "BufWinEnter"}, {
    pattern = { "*.sql", "*.lua" },
    command = [[:iab *@ -- *| :iab tod -- TODO:| :iab fi -- FIX:| :iab inp -- INPROGRESS:| :iab que -- QUESTION:| :iab cav -- CAVEAT:| :iab rol -- ROLLUP_KEYS:<CR>--<TAB>* INDEX FIELD: Primary Key<CR>--<TAB>* MERGE FIELD: Field To Merge On If Row Needs To Be Updated<CR>--<TAB>* ROLLUP LOAD TYPE:<CR>--<TAB><TAB>* merge - Insert/replace records in Redshift without consideration of merge field<CR>--<TAB><TAB>* updateRecords - Insert/replace records in Redshift where merge field greater than record merge field in Redshift<CR>--<TAB><TAB>* refresh - Delete all records in Redshift table then replace with rollup results<CR>--<TAB>* TABLE NAME: Descriptive Table Name<CR>--<TAB>* PERIODICITY: How Often To Run Query<CR>--<TAB>* LOOK BACK WINDOW: Run Query For Last X Hours<CR>--<TAB>* BACKFILL: Date To Backfill To<ESC>10k/: <CR>wvg_h| :iab tit -- TITLE:| :iab lin -- LINK:| :iab abo -- ABOUT:]],
    group = expandKey
})
vim.api.nvim_create_autocmd({"BufEnter", "BufWinEnter"}, {
    pattern = "*.py",
